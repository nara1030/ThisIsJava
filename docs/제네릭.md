...
* 정적 메소드임에도 객체를 생성해서 사용한다.
	...
	* 단, 클라이언트(생성의 주체) 관련해서 객체 의존성을 한 번 생각해볼 필요가 있을 것 같다.
		* [객체에 메시지를 보내라](http://woowabros.github.io/files/2019-05-02/precourse_2nd_feedback.pdf)  
		<img src="../img/java_refactoring_method_call.png" width="480" height="300"></br>
			* 즉, 매개변수로 객체를 넣지 말고 해당 클래스의 멤버를 사용하도록 메소드를 위치(설계)시켜라
			* MVC 패턴 상기
				* Controller에서 Service 메소드 호출, Service에서 Dao 메소드 호출
