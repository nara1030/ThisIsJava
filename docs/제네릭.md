...
* [Java Generics PECS](https://howtodoinjava.com/java/generics/java-generics-what-is-pecs-producer-extends-consumer-super/)
	* PECS is a term first coined by Joshua Bloch in his book `Effective Java`.
	* 비교  
		```java
		boolean addAll(Collection<? extends E> c);
		public static <T> boolean addAll(Collection<? super T> c, T ... elements);
		```
	* Understanding `<? extends T>`: PE(Producer extends)
		* To more relate it to real life terms, let's use an analogy of a basket of fruits(i.e. collection of fruits).
		* so that we can write generic code like this:  
			```java
			Fruit get = fruits.get(0);
			```
		* In above case, we need to declare the collection of fruits as `List<? extends Fruit>`.
		* [예제 코드](https://github.com/nara1030/ThisIsJava/blob/master/thisIsJava_ch13/src/test/pecs/ex_02/Producer_Extends.java)
	* Understanding `<? super T>`
		...
		* [예제 코드](https://github.com/nara1030/ThisIsJava/blob/master/thisIsJava_ch13/src/test/pecs/ex_02/Consumer_Super.java)

* 포인터로 연결 리스트 만들기(출처: 자료구조와 함께 배우는 알고리즘 입문_자바편)  
	```java
	// 검색을 수행하는 search 메소드: 선형 검색
